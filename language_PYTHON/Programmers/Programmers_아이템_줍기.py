# 복습 횟수:0, 03:00:00, 복습필요3
from collections import deque

def solution(rectangle, characterX, characterY, itemX, itemY):
    answer = 0
    characterX, characterY = characterY, characterX
    itemX, itemY = itemY, itemX

    # 제한사항에서 모든 좌표값은 1 이상 50 이하라고 했고 2배의 좌표를 그려야 하므로 102*102 크기의 2차원 배열 선언
    field = [[2] * 102 for i in range(102)]
    
    # 직사각형 그리기
    for r in rectangle:
    	# 모든 좌표값 2배
        y1, x1, y2, x2 = map(lambda x: x*2, r)
        x1 = 102 - x1 - 1
        x2 = 102 - x2 - 1
    ...
    q.append([102 - (characterX * 2) - 1, characterY * 2])
    ...
    while q:
        ...
        if x == 102 - (itemX * 2) - 1 and y == itemY * 2:
