...
def bfs(r, c, s_dir):
    ...
    while queue:
        ...
        # 만약 도착지점에 도착했다면 횟수 저장 후 종료
        if r == end_r - 1 and c == end_c - 1 and c_dir == end_dir:
            result = count
            return  
        for i in range(1, 3 + 1): #1,2,3칸 차례로 이동하기
            nr = r + (dr[c_dir] * i)
            nc = c + (dc[c_dir] * i)   
            #범위 내에서 움직이기 and 방문하지 않았을 경우
            if(0 <= nr < M and 0 <= nc < N and not visited[nr][nc][c_dir]):
                if miro[nr][nc] == 1: #만약 1이 껴있으면 멈춤
                    break
                visited[nr][nc][c_dir] = True
                queue.append([nr, nc, c_dir, count + 1])
        
        if c_dir == 1 or c_dir == 2: #현재가 동,서 인 경우
